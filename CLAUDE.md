...
- **Track A — Platform Core** (Sprint 1: complete): `Result<T,E>` ✅ · Typed IDs ✅ · `PlatformError` ✅ · `Clock` ✅ · Value Objects ✅ (resolved by decision, no artifact) · Domain Events ✅ · Validation ✅ (ADR-021) · Core Tests ✅. Sprint 2 (Platform Services) deferred (ADR-023) in favor of Sprint 3 (AI Runtime).
...
- **Track C — AI Runtime** (in progress): `ai.provider` interface ✅ (ADR-024) — provider-neutral contract (`complete(AiPrompt) -> Result<AiCompletion, PlatformError>`), plus deterministic `fixed`/`failing` test doubles; no vendor SDK, no concrete provider implementation. Blocked past this point on a provider choice and API credentials — neither is something an AI assistant can decide or obtain unilaterally (see `PROJECT_STATE.md`).
- **Track D — Engineering Intelligence** (in progress, resequenced ahead of Identity/Workspace — see ADR-025, ADR-026): `intelligence.repository` ✅ (ADR-025) — deterministic module structure and internal package-dependency scanning (`RepositoryScanner`), no AI involved. `intelligence.architecture` ✅ (ADR-026) — circular-dependency detection (`CycleDetector`, Tarjan's SCC algorithm), the first real consumer of `repository`'s facts. Both dogfooded against this repository itself. Chosen specifically because — unlike Track C past its interface, or publishing this repository — neither needs AI provider credentials or GitHub access to build and verify; real, unblocked engineering value while Tracks C and repository publication wait on external input.

**Naming note:** `core.id.RepositoryId` (a typed identifier for a future domain "Repository" entity — a tracked, registered repository once Identity/Workspace exist) and `intelligence.repository` (concrete Git/filesystem structure scanning, works on any local checkout today, no registration needed) are deliberately unlinked for now — the same English word, two different concepts at different layers. `RepositorySnapshot` carries no `RepositoryId` because nothing yet assigns one to a scanned filesystem path. Revisit when Workspace/Repository registration is real; don't let the shared name imply a connection that doesn't exist yet.
...
Full sprint sequence as originally documented in `ARCHITECTURE_STATUS.md`: Sprint 0 (Foundation, done) → Sprint 1 (Platform Core, done) → Platform Services → AI Runtime → Identity → Workspace → Repository Intelligence → Knowledge Engine → Reasoning Engine → Decision Engine → Automation Engine → Engineering Intelligence → Dashboard → v1.0. **Resequenced by implementation evidence** (ADR-023, ADR-025, ADR-026), per this section's own mandate to verify sequencing against reality rather than follow it blindly: Platform Services deferred (no concrete caller existed); Repository/Architecture Intelligence started ahead of Identity/Workspace/full AI Runtime specifically because they need neither AI credentials nor GitHub access, unlike everything else currently blocked. This is an evidence-based reordering, not an abandonment of the plan — Identity, Workspace, Platform Services, and the rest of AI Runtime remain next once their respective blockers clear.
